...
function glossary_replace_handler(event){if($("input[@name=glossary_replace]:checked").val()=='superscript'){$("input[@name=glossary_superscript]").parents("div.glossary_superscript").show()}else $("input[@name=glossary_superscript]").parents("div.glossary_superscript").hide();if($("input[@name=glossary_replace]:checked").val()=='icon'){$("input[@name=glossary_icon]").parents("div.glossary_icon").show()}else $("input[@name=glossary_icon]").parents("div.glossary_icon").hide()}
function glossary_link_related_handler(event){if($("input[@name=glossary_link_related]:checked").val()==1){$("input[@name=glossary_link_related_how]").parents("div.glossary_link_related_how").show()}else{$("input[@name=glossary_link_related_how]").val(0);$("input[@name=glossary_link_related_how]").parents("div.glossary_link_related_how").hide()}};if(Drupal.jsEnabled)$(document).ready(function(){glossary_replace_handler();glossary_link_related_handler();$("input[@name=glossary_replace]").bind("click",glossary_replace_handler);$("input[@name=glossary_link_related]").bind("click",glossary_link_related_handler)});if(document.layers){pre='document.';post=''};if(document.getElementById){pre='document.getElementById("';post='").style'};if(document.all){pre='document.all.';post='.style'}
...
function hide_layer(layer){var truc=pre+layer+post;eval(pre+layer+post).visibility='hidden';eval(pre+layer+post).position='absolute'}
function hide_folder(folder){hide_folding_layer(folder);show_layer('show'+folder);scrollBy(0,0)}
function show_folder(folder){show_layer('hide'+folder);hide_layer('show'+folder);show_layer('fold'+folder);scrollBy(0,0);var i;for(i=1;layer_exists('fold'+folder+'_'+i);++i)show_layer('show'+folder+'_'+i)}
function show_folder_completely(folder){show_layer('hide'+folder);hide_layer('show'+folder);show_layer('fold'+folder);scrollBy(0,0);var i;for(i=1;layer_exists('fold'+folder+'_'+i);++i)show_folder_completely(folder+'_'+i)}
function hide_folding_layer(folder){var i;for(i=1;layer_exists('fold'+folder+'_'+i);++i)hide_folding_layer(folder+'_'+i);hide_layer('hide'+folder);hide_layer('show'+folder);hide_layer('fold'+folder);scrollBy(0,0)}
function fold_document(){var i,folder='1';for(i=1;layer_exists('fold'+folder+'_'+i);++i)hide_folder(folder+'_'+i)}
function unfold_document(){var i,folder='1';for(i=1;layer_exists('fold'+folder+'_'+i);++i)show_folder_completely(folder+'_'+i)};

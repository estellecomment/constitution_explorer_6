...
Drupal.behaviors.fb_connect=function(context){jQuery("a[href^='"+Drupal.settings.basePath+"logout']",context).click(FB_Connect.logoutHandler);FB_Connect.enablePopups(context);var events=jQuery(document).data('events');if(!events||!events.fb_session_change)jQuery(document).bind('fb_session_change',FB_Connect.sessionChangeHandler)};FB_Connect=function(){};FB_Connect.sessionChangeHandler=function(context,status){jQuery('.block-fb_connect').ajaxStart(function(){jQuery(this).html('<div class="progress fb_connect_pb"><div class="bar"><div class="filled"></div></div></div>')}).ajaxStop(function(){jQuery(this).html('')});FB_JS.sessionChangeHandler(context,status)};FB_Connect.logoutHandler=function(event){if(typeof FB!='undefined'){FB.logout(function(){});FB_JS.deleteCookie('fbs_'+FB._apiKey,'/','');FB_JS.deleteCookie('fbs_'+Drupal.settings.fb.apikey,'/','')};if(FB.getUser()){Drupal.settings.fb.reload_url=Drupal.settings.fb_connect.front_url;return false}};FB_Connect.centerPopups=function(){var scrollTop=$(window).scrollTop();$('.fb_dialog:not(.fb_popup_centered)').each(function(){var offset=$(this).offset();if(offset.left==0);else if(offset.top<0);else if(offset.top<scrollTop){$(this).css('top',offset.top+scrollTop+'px');$(this).addClass('fb_popup_centered')}})};FB_Connect.enablePopups=function(context){$('.fb_fbml_popup:not(.fb_fbml_popup-processed)',context).addClass('fb_fbml_popup-processed').prev().each(function(){this.fbml_popup=$(this).next().html();this.fbml_popup_width=parseInt($(this).next().attr('width'));this.fbml_popup_height=parseInt($(this).next().attr('height'));$(this).next().remove()}).bind('click',function(e){var popup,size=FB.UIServer.Methods["fbml.dialog"].size;if(this.fbml_popup_width)size.width=this.fbml_popup_width;if(this.fbml_popup_height)size.height=this.fbml_popup_height;FB.UIServer.Methods['fbml.dialog'].size=size;var markup=this.fbml_popup;if($(this).attr('title'))markup='<fb:header icon="true" decoration="add_border">'+$(this).attr('title')+'</fb:header>'+this.fbml_popup;var dialog={method:'fbml.dialog',display:'dialog',fbml:markup,width:this.fbml_popup_width,height:this.fbml_popup_height},popup=FB.ui(dialog,function(response){console.log(response)});window.setInterval(FB_Connect.centerPopups,500);e.preventDefault()}).parent().show()};
FB_Stream=function(){};FB_Stream.stream_publish=function(json){var data_array=Drupal.parseJson(json),len=data_array.length;for(var i=0;i<len;i++){var data=data_array[i];data.method='stream.publish';FB.ui(data)}};
...
;
;
$(document).ready(function(){if(!$('#admin-menu').length)return;if(Drupal&&Drupal.settings&&Drupal.settings.admin_menu){if(Drupal.settings.admin_menu.margin_top)$('body').addClass('admin-menu');if(Drupal.settings.admin_menu.position_fixed)$('#admin-menu').css('position','fixed');if(Drupal.settings.admin_menu.tweak_tabs){$('ul.tabs.primary li').each(function(){$(this).addClass('admin-menu-tab').appendTo('#admin-menu > ul')});$('ul.tabs.secondary').appendTo('#admin-menu > ul > li.admin-menu-tab.active').removeClass('secondary')};if(Drupal.settings.admin_menu.tweak_modules)$('#system-modules fieldset:not(.collapsed), #system-modules-1 fieldset:not(.collapsed)').addClass('collapsed')};if($.browser.msie&&parseInt(jQuery.browser.version)==6)$('#admin-menu li').hover(function(){$(this).addClass('iehover')},function(){$(this).removeClass('iehover')});$('#admin-menu li').hover(function(){clearTimeout(this.sfTimer);$('> ul',this).css({left:'auto',display:'block'}).parent().siblings('li').children('ul').css({left:'-999em',display:'none'})},function(){var uls=$('> ul',this);this.sfTimer=setTimeout(function(){uls.css({left:'-999em',display:'none'})},400)})});
...
Drupal.ModuleFilter=Drupal.ModuleFilter||{};Drupal.ModuleFilter.textFilter='';Drupal.ModuleFilter.timeout;Drupal.ModuleFilter.tabs={};Drupal.ModuleFilter.enabling={};Drupal.ModuleFilter.disabling={};Drupal.behaviors.moduleFilter=function(){$('#edit-module-filter-name').focus();$('#module-filter-squeeze').css('min-height',$('#module-filter-tabs').height());$('#module-filter-left a.project-tab').each(function(i){Drupal.ModuleFilter.tabs[$(this).attr('id')]=new Drupal.ModuleFilter.Tab(this)});$('a.anchor',$('#module-filter-left')).remove().prependTo('#module-filter-tabs');$('#edit-module-filter-name').keyup(function(){if(Drupal.ModuleFilter.textFilter!=$(this).val()){Drupal.ModuleFilter.textFilter=this.value;if(Drupal.ModuleFilter.timeout)clearTimeout(Drupal.ModuleFilter.timeout);Drupal.ModuleFilter.timeout=setTimeout('Drupal.ModuleFilter.filter("'+Drupal.ModuleFilter.textFilter+'")',500)}});Drupal.ModuleFilter.showEnabled=$('#edit-module-filter-show-enabled').is(':checked');$('#edit-module-filter-show-enabled').change(function(){Drupal.ModuleFilter.showEnabled=$(this).is(':checked');Drupal.ModuleFilter.filter($('#edit-module-filter-name').val())});Drupal.ModuleFilter.showDisabled=$('#edit-module-filter-show-disabled').is(':checked');$('#edit-module-filter-show-disabled').change(function(){Drupal.ModuleFilter.showDisabled=$(this).is(':checked');Drupal.ModuleFilter.filter($('#edit-module-filter-name').val())});Drupal.ModuleFilter.showRequired=$('#edit-module-filter-show-required').is(':checked');$('#edit-module-filter-show-required').change(function(){Drupal.ModuleFilter.showRequired=$(this).is(':checked');Drupal.ModuleFilter.filter($('#edit-module-filter-name').val())});Drupal.ModuleFilter.showUnavailable=$('#edit-module-filter-show-unavailable').is(':checked');$('#edit-module-filter-show-unavailable').change(function(){Drupal.ModuleFilter.showUnavailable=$(this).is(':checked');Drupal.ModuleFilter.filter($('#edit-module-filter-name').val())});if(Drupal.settings.moduleFilter.visualAid==1)$('#projects tbody td.checkbox input').change(function(){if($(this).is(':checked')){Drupal.ModuleFilter.updateVisualAid('enable',$(this).parents('tr'))}else Drupal.ModuleFilter.updateVisualAid('disable',$(this).parents('tr'))});var url=document.location.toString();if(url.match('#')){var anchor='#'+url.split('#')[1];$('a[href="'+anchor+'"]').click()}else if(Drupal.ModuleFilter.activeTab==undefined)Drupal.ModuleFilter.activeTab=Drupal.ModuleFilter.tabs['all-tab']};Drupal.ModuleFilter.visible=function(checkbox){if(Drupal.ModuleFilter.showEnabled)if($(checkbox).is(':checked')&&!$(checkbox).is(':disabled'))return true;if(Drupal.ModuleFilter.showDisabled)if(checkbox.size()&&(!$(checkbox).is(':checked')&&!$(checkbox).is(':disabled')))return true;if(Drupal.ModuleFilter.showRequired)if($(checkbox).is(':checked')&&$(checkbox).is(':disabled'))return true;if(Drupal.ModuleFilter.showUnavailable)if(!checkbox.size()||(!$(checkbox).is(':checked')&&$(checkbox).is(':disabled')))return true;return false};Drupal.ModuleFilter.filter=function(string){var stringLowerCase=string.toLowerCase(),flip='odd';if(Drupal.ModuleFilter.activeTab.id=='all-tab'){var selector='#projects tbody tr td > strong'}else var selector='#projects tbody tr.'+Drupal.ModuleFilter.activeTab.id+'-content td > strong.project-name';$(selector).each(function(i){var $row=$(this).parent().parent(),module=$(this).text(),moduleLowerCase=module.toLowerCase();if(moduleLowerCase.match(stringLowerCase)){if(Drupal.ModuleFilter.visible($('td.checkbox input',$row))){$row.removeClass('odd even');$row.addClass(flip);$row.show();flip=(flip=='odd')?'even':'odd'}else $row.hide()}else $row.hide()})};Drupal.ModuleFilter.Tab=function(element){this.id=$(element).attr('id');this.element=element;$(this.element).click(function(){Drupal.ModuleFilter.tabs[$(this).attr('id')].setActive()})};Drupal.ModuleFilter.Tab.prototype.setActive=function(){if(Drupal.ModuleFilter.activeTab){$(Drupal.ModuleFilter.activeTab.element).parent().removeClass('active')}else $('#all-tab').parent().removeClass('active');Drupal.ModuleFilter.activeTab=this;$(Drupal.ModuleFilter.activeTab.element).parent().addClass('active');Drupal.ModuleFilter.activeTab.displayRows();$('#edit-module-filter-name').val('');$('#edit-module-filter-name').focus()};Drupal.ModuleFilter.Tab.prototype.displayRows=function(){var flip='odd',selector=(Drupal.ModuleFilter.activeTab.id=='all-tab')?'#projects tbody tr':'#projects tbody tr.'+this.id+'-content';$('#projects tbody tr').hide();$('#projects tbody tr').removeClass('odd even');$(selector).each(function(i){if(Drupal.ModuleFilter.visible($('td.checkbox input',$(this)))){$(this).addClass(flip);flip=(flip=='odd')?'even':'odd';$(this).show()}});if(typeof Drupal.ModuleFilter.enabledCount=='function')Drupal.ModuleFilter.enabledCount(Drupal.ModuleFilter.activeTab)};Drupal.ModuleFilter.Tab.prototype.updateEnabling=function(amount){this.enabling=this.enabling||0;this.enabling+=amount;if(this.enabling==0)delete(this.enabling)};Drupal.ModuleFilter.Tab.prototype.updateDisabling=function(amount){this.disabling=this.disabling||0;this.disabling+=amount;if(this.disabling==0)delete(this.disabling)};Drupal.ModuleFilter.Tab.prototype.updateVisualAid=function(){var visualAid='';if(this.enabling!=undefined)visualAid+='<span class="enabling">'+Drupal.t('+@count',{'@count':this.enabling})+'</span>';if(this.disabling!=undefined)visualAid+='<span class="disabling">'+Drupal.t('-@count',{'@count':this.disabling})+'</span>';if(!$('span.visual-aid',$(this.element)).size()&&visualAid!='')$(this.element).prepend('<span class="visual-aid"></span>');$('span.visual-aid',$(this.element)).empty().append(visualAid)};Drupal.ModuleFilter.updateVisualAid=function(type,row){var classes=row.attr('class').split(' ');for(var i in classes){var id=classes[i].substring(0,classes[i].length-8);if(Drupal.ModuleFilter.tabs[id]!=undefined)break};if(Drupal.ModuleFilter.activeTab.id=='all-tab'){var allTab=Drupal.ModuleFilter.activeTab,projectTab=Drupal.ModuleFilter.tabs[id]}else{var allTab=Drupal.ModuleFilter.tabs['all-tab'],projectTab=Drupal.ModuleFilter.activeTab};var name=$('td > strong',row).text();switch(type){case'enable':if(Drupal.ModuleFilter.disabling[id+name]!=undefined){delete(Drupal.ModuleFilter.disabling[id+name]);allTab.updateDisabling(-1);projectTab.updateDisabling(-1);row.removeClass('disabling')}else{Drupal.ModuleFilter.enabling[id+name]=name;allTab.updateEnabling(1);projectTab.updateEnabling(1);row.addClass('enabling')};break;case'disable':if(Drupal.ModuleFilter.enabling[id+name]!=undefined){delete(Drupal.ModuleFilter.enabling[id+name]);allTab.updateEnabling(-1);projectTab.updateEnabling(-1);row.removeClass('enabling')}else{Drupal.ModuleFilter.disabling[id+name]=name;allTab.updateDisabling(1);projectTab.updateDisabling(1);row.addClass('disabling')};break};allTab.updateVisualAid();projectTab.updateVisualAid()};
